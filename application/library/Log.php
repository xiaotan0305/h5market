...
class Log
{
     ...
     */
    public function __construct($task = 'defaultlog')
    {
        $config = Yaf_Registry::get('application');
        $this->_dir = $config['application']['logdir'];
        $dir = ($this->_dir) . $task . '/' . date('Y') . '/' . date('m');
        ...
        //组合文件名
        $this->_logfile = $dir . '/' . $task . date('Ymd') . '.log';
    }
    ...
    public function fileWrite($str)
    {
        ...
        //加个换行用
        $res = fwrite($handle, date('Y-m-d H:i:s').' '.$str."\n");
        fclose($handle);
    ...
    public function clearExpireLog()
    {
        ...
        foreach (glob($this->_dir.'*'.'/'.'*'.'/'.'*', GLOB_ONLYDIR) as $logthis) {
            //截掉最后 7 位的年和月数，再加上要删除的年月数
            $dir2delete = substr($logthis, 0, -7).$yeardelete.'/'.$monthdelete;
        ...
        //处理空的年文件夹
        foreach (glob($this->_dir.'*'.'/'.'*', GLOB_ONLYDIR) as $logthis) {
            if ($yeardelete == $yearnow && substr($logthis, -4) < $yearnow && is_dir($logthis)) {
                $msg = $logthis;
                if (rmdir($logthis)) {
                    $msg .= ' deleted.';
                } else {
